...
/**
 * Updates all fake buyers' prices to a random value between 1% and 2.5% above the latest market price.
 */
export async function updateFakeBuyerPrices() {
  try {
    ...
    for (const buyer of buyers) {
      // Random percent between 1% and 2.5%
      const percent = 1 + Math.random() * 1.5; // 1% to 2.5%
